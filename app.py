# -*- coding: utf-8 -*-
# ================================================
# AI Swing Prediction System V5 - SPY Dual Probability
# Main: P(10D > +2%) drives signal
# Aux:  P(10D > +5%) strength gauge (not a trigger)
# ================================================
import time
import numpy as np
import pandas as pd
...
def load_data():
    ...
    df["Target_Strong"] = (df["Future_Return"] > TARGET_STRONG).astype(int)
    return df
...
def run_walk_forward(df):
    ...
    strong_series = pd.Series(dtype=float)
    for i in range(N_FOLDS):
        test_start = n - (N_FOLDS - i) * test_size
        test_end = test_start + test_size
        train_end = test_start - HORIZON
        idx = train_df.index[test_start:test_end]
        X_tr = train_df[FEATURES].iloc[:train_end]
        X_te = train_df[FEATURES].iloc[test_start:test_end]

        m = make_model()
        m.fit(X_tr, train_df["Target"].iloc[:train_end])
        prob_series = pd.concat([prob_series,
            pd.Series(m.predict_proba(X_te)[:, 1], index=idx)])

        ms = make_model()
        ms.fit(X_tr, train_df["Target_Strong"].iloc[:train_end])
        strong_series = pd.concat([strong_series,
            pd.Series(ms.predict_proba(X_te)[:, 1], index=idx)])

    latest_row = feat_df.iloc[[-1]]
    fm = make_model()
    fm.fit(train_df[FEATURES], train_df["Target"])
    latest_prob = float(fm.predict_proba(latest_row[FEATURES])[0][1])

    fs = make_model()
    fs.fit(train_df[FEATURES], train_df["Target_Strong"])
    latest_strong = float(fs.predict_proba(latest_row[FEATURES])[0][1])

    return prob_series, strong_series, train_df, feat_df, latest_prob, latest_strong
...
prob_series, strong_series, train_df, feat_df, latest_prob, latest_strong = run_walk_forward(df)
...
strong_base = train_df["Target_Strong"].mean()
strong_pctile = float((strong_series < latest_strong).mean()) * 100
...
with tab1:
    ...
    st.divider()
    st.subheader("波段強度計 (>5% 稀有事件)")
    s1, s2, s3 = st.columns(3)
    s1.metric("10日漲>5% 概率", "%.1f%%" % (latest_strong * 100))
    s2.metric("其歷史基準率", "%.1f%%" % (strong_base * 100))
    s3.metric("歷史百分位", "%.0f%%" % strong_pctile)
    st.caption("僅作波段強度參考, 不觸發買賣。百分位越高代表當前暴反結構越罕見地強。")

    st.divider()
    st.subheader("樣本外驗證 (Walk-Forward, 主模型 >2%)")
    ...
    if latest_prob > PROB_THRESHOLD and model_valid:
        ...
        if latest_strong >= STRONG_ALERT:
            st.warning("強勢波段模式: >5%% 概率 %.1f%% 遠超基準率 %.1f%% — 可考慮止盈上調至 +3.5%%, 或 +2%% 達標後只平一半、餘倉改追蹤止損" % (latest_strong * 100, strong_base * 100))
        else:
            st.caption("標準波段模式: 嚴格 +2% 止盈離場")
...
with tab2:
    ...
    if len(tdf) == 0:
        ...
        ax.set_title("SPY Equity Curve (OOS trades)")
        ax.set_xlabel("Trade #")
        ax.grid(alpha=0.3)
        st.pyplot(fig)
...
st.divider()
st.caption("本系統僅供研究參考, 不構成投資建議。>5% 概率僅為強度參考, 不構成獨立信號。")
